...
plot_gen <- function(mydata, method){
    # Simple correlation matrix calculation
mydata.cor <- cor(mydata, method = method, use = "complete.obs")
...
# Determine connectivity!
# filtering
selector <- ((abs(mydata.cor.ro.1) <= 0.35 ) * 1)
...
# Repeat selector matrix, this time for strength of relationships
thickness <- ((abs(mydata.cor.ro.1) <= 0.35 ) * 1)
...
#Test
plot_gen(mydata = as.matrix(WDBC[,3:32]), method = "pearson")
...
shinyApp(
  ...
  server = function(input, output) {
    output$feature_plot = renderPlotly({
        req(input$file)
        ggplotly(
            plot_gen(mydata = read.csv(input$file$datapath) %>% dplyr::select(where(is.numeric)), method = input$cor_method), height = 800
        )
    })
  }
)
